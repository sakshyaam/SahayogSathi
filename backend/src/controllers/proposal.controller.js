...
const getPostProposals = asyncHandler(async (req, res) => {
  const { postId } = req.params;
  ...
  // Only post owner can see all proposals
  if (post.postedBy.toString() !== req.user._id.toString()) {
...
const acceptProposal = asyncHandler(async (req, res) => {
  ...
  if (proposal.post.postedBy.toString() !== req.user._id.toString()) {
  ...
  // Update post status and set acceptedProposal
  const post = await Post.findById(proposal.post._id);
  if (!post) {
  ...
  // Reject all other pending proposals for this post
  await Proposal.updateMany(
    { post: post._id, _id: { $ne: proposal._id }, status: "pending" },
    { status: "rejected" }
  );

  // Ensure conversation exists between owner and helper
